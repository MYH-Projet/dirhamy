...
export const removeTransaction = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const transactionToDelete = await transactionServices.checkExistAndOwnershipTransaction(transactionId,userId);
        const message = await transactionServices.deleteTransactionService(transactionId,transactionToDelete);
...
export const updateTransaction = async (req: AuthRequest, res: Response) => {
    ...
    try {
        const existingTransaction = await transactionServices.checkExistAndOwnershipTransactionCategory(transactionId,userId,req.body.categorieId);
        const result = await transactionServices.updateTransaction(transactionId,existingTransaction,req.body);
        return res.status(200).json({
